...
<head>
    ...
    <style>
        ...
        .hover\:bg-verde-profundo:hover {
            background-color: #3A7D4A;
        }

        .hover\:bg-verde-institucional:hover {
            background-color: #7BC47F;
        }
...
<body class="h-full bg-gris-claro antialiased">
    ...
    <div x-data="workstation()" @toast.window="mostrarToast($event.detail)" class="min-h-full flex flex-col">
        <div x-init="$nextTick(() => cargarUsuario())">
            <!-- Barra superior -->
            <header
                class="fixed top-0 left-0 right-0 h-14 z-40 bg-verde-profundo shadow-lg flex items-center px-4 gap-3">
                <!-- Logo más grande -->
                <img src="/images/logoconsejo_sinfondo.png" alt="Consejo de la Magistratura"
                    class="h-11 w-auto object-contain hidden sm:block">
                ...
                <div class="hidden sm:flex items-center gap-2 text-white/95 text-xs">
                    ...
                    <span
                        class="px-2 py-0.5 rounded-full bg-verde-institucional/40 text-white text-[10px] uppercase tracking-wide"
                        x-text="usuario?.rol || ''"></span>
                </div>
                <button @click="cerrarSesion"
                    class="text-white/90 hover:text-white transition flex items-center gap-1.5 text-xs"
                ...
                <nav class="p-3 space-y-1 text-sm">
                    ...
                    <template x-if="usuario?.rol === 'ADMINISTRADOR'">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="px-3 pt-2 pb-1 text-[10px] uppercase tracking-wider
                    text-white/50">
                                Administración
                            </div>

                            <a href="/administrador/dashboard" class="flex items-center gap-3 px-3 py-2.5 rounded-lg
                  hover:bg-white/20 transition">
                            ...
                            <a href="/administrador/usuarios" class="flex items-center gap-3 px-3 py-2.5 rounded-lg
                  hover:bg-white/20 transition">

                                <i class="fa-solid fa-users w-5 text-center text-verde-institucional"></i>

                                <span>Usuarios</span>

                            </a>

                            <a href="/administrador/monitoreo" class="flex items-center gap-3 px-3 py-2.5 rounded-lg
                  hover:bg-white/20 transition">

                                <i class="fa-solid fa-binoculars w-5 text-center text-verde-institucional"></i>

                                <span>Monitoreo de expedientes</span>

                            </a>

                            <a href="/administrador/feriados" class="flex items-center gap-3 px-3 py-2.5 rounded-lg
                  hover:bg-white/20 transition">

                                <i class="fa-solid fa-calendar-days w-5 text-center text-verde-institucional"></i>
            ...
            <div class="fixed bottom-4 right-4 z-50 space-y-2 w-full max-w-sm">
                <template x-for="t in toasts" :key="t.id">
                    <div x-show="t.visible" x-transition
                        class="rounded-lg border shadow-lg text-sm p-3.5 flex items-start gap-3" :class="{
                        ...
                        <div>
                            ...
                            <pre v-if="t.detalle" x-show="t.detalle" class="mt-1 text-xs whitespace-pre-line font-sans"
                                x-text="t.detalle"></pre>
                        </div>
